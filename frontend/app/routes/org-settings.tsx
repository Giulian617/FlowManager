  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <header className="flex flex-col gap-2">
        ...
        <h1 className="text-3xl font-semibold text-slate-900">Preferences</h1>
      </header>

      <section className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h2 className="text-sm font-semibold text-slate-900">Notification Channels</h2>
            ...
            <div key={label} className="border-b border-slate-100 last:border-0">
              ...
              {isExpanded && enabled && (
                <div className="px-6 pb-4 bg-slate-50 border-t border-slate-100">
                  <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 pt-3 mb-3">Notify me when</p>
                  <div className="space-y-3">
                    {TRIGGERS.map(([triggerLabel, key]) => (
                      <div key={key} className="flex items-center justify-between py-1">
                        <span className="text-sm text-slate-600">{triggerLabel}</span>
                        <Toggle
                  ...
                  <div className="pt-3 mt-2 border-t border-slate-200 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-600">Deadline reminder</span>
                      <Toggle
                    ...
                    {settings.deadline && (
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-400">Notify</span>
                        <select
