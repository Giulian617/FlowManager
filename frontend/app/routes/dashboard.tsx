...
const PROJECT_DATA: Record<string, {
  ...
  progress: number
  deadline: string
  recentActivity: { title: string; detail: string; time: string; type: "bug" | "task" | "info" | "warning" }[]
}> = {
  "1": {
    openItems: 34, teams: 3, members: 9, progress: 62,
    deadline: "2026-07-30",
    recentActivity: [
      { title: "Login bug reported", detail: "Login button unresponsive on Safari.", time: "4h ago", type: "bug" as const },
      { title: "Implement user settings", detail: "Task added to current sprint.", time: "2h ago", type: "task" as const },
      { title: "Q3 Delivery Epic", detail: "New epic created for Q3 delivery.", time: "Yesterday", type: "info" as const },
      { title: "User onboarding story", detail: "User story refined and estimated.", time: "2 days ago", type: "warning" as const },
    ],
  },
  "2": {
...
const activityIcons = {
  ...
  info:    <Zap className="h-4 w-4 flex-none" />,
  warning: <BookOpen className="h-4 w-4 flex-none" />,
}
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {!data ? (
        ...
        <>
          {/* Stat cards + Recent activity în același grid */}
