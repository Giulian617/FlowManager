...
import {
  Bug, ArrowLeft, ChevronDown, Search, UserCircle,
  AlertCircle, Send, Paperclip, Calendar, Link2, Lock, X } from "lucide-react"
...
function SelectDropdown({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="text-sm">{value ? (renderSelected ? renderSelected(value) : value) : <span className="text-slate-400 text-sm">{placeholder ?? "Select…"}</span>}</span>
...
function AssigneeMultiDropdown({ value, onChange }: {
  value: string[]
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none transition hover:border-slate-400 focus:ring-2 focus:ring-slate-200"
      >
        <div className="flex items-center gap-2">
          <UserCircle className="h-4 w-4 text-slate-400" />
          <span className="text-sm text-slate-400">
            {value.length === 0 ? "Add assignee…" : "Add more…"}
          </span>
        </div>
            ...
            {filtered.map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  {ini ? (
                    <div className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-100 text-blue-900 border border-blue-900 text-[10px] font-semibold flex-none">{ini}</div>
                  ) : (
                    <div className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-slate-500 flex-none"><UserCircle className="h-3.5 w-3.5" /></div>
                  )}
...
function ParentField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const filtered = parentOptions.filter(
    (p) => p.id.includes(typed.replace("#", "")) || p.title.toLowerCase().includes(typed.toLowerCase())
  )
  ...
  return (
    <div ref={ref} className="relative">
      <div className="flex items-center rounded-xl border border-slate-200 bg-white transition hover:border-slate-400 focus-within:ring-2 focus-within:ring-slate-100 overflow-hidden">
        ...
        <input
          value={typed}
          onChange={(e) => { setTyped(e.target.value); onChange(e.target.value); setOpen(true) }}
        ...
        {comments.map((c) => {
          ...
          return (
            <div key={c.id} className="flex gap-3">
              <div className="flex h-8 w-8 flex-none items-center justify-center rounded-full bg-blue-100 text-blue-900 border border-blue-900 text-xs font-semibold">{ini}</div>
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 rounded-2xl border border-slate-200 bg-white overflow-hidden focus-within:ring-2 focus-within:ring-slate-200 focus-within:border-slate-400 transition">
          ...
          <div className="flex items-center justify-between border-t border-slate-100 px-3 py-2">
            <button type="button" className="inline-flex items-center gap-1.5 rounded-xl px-2 py-1 text-xs text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition">
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-50 border border-rose-200">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm space-y-3.5">
            ...
            <div>
              ...
              <SelectDropdown
                ...
                error={!severityOk}
                renderOption={(v) => (
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold uppercase tracking-[0.15em] ${priorityMeta[v as keyof typeof priorityMeta]?.className}`}>
                    {v}
                  </span>
                )}
                renderSelected={(v) => (
