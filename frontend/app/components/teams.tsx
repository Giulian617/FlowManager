    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl p-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onConfirm} className="flex-1 rounded-xl bg-slate-800 dark:bg-slate-200 px-4 py-2.5 text-sm font-semibold text-white dark:text-slate-900 transition hover:bg-slate-700 dark:hover:bg-slate-300">Delete</button>
        </div>
      </div>
    </div>
  )
...
export default function Teams({ mode }: { mode: "org" | "project" }) {
  ...
  useEffect(() => {
    ...
    if (!storedOrgId) { navigate("/select-org"); return }
    if (mode === "project" && !storedProjectId) { navigate("/org/projects"); return }
                  ...
                  {canModify && (
                    <div className="absolute top-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(team) }}
                        className="inline-flex h-7 w-7 items-center justify-center rounded-xl border border-rose-100 dark:border-rose-900 bg-white dark:bg-slate-700 text-rose-400 dark:text-rose-500 transition hover:bg-rose-50 dark:hover:bg-rose-950/40 hover:text-rose-600 dark:hover:text-rose-400" title="Delete">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                  )}
                  <div className={canModify ? "pr-14" : ""}>
                    <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{team.name}</h2>
